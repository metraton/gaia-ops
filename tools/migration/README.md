...
| 02 | Notes | `~/.claude/projects/-home-jorge-ws-me/memory/*.md` | `notes` (+`notes_fts`) |
| 03 | Context sections | `.claude/project-context/project-context.json` | `context_sections` |
...
./migrate_02_notes.sh                    # 28 .md (MEMORY.md excluido)
./migrate_03_context_sections.sh         # 12 secciones
...
| 02 notes | `INSERT OR IGNORE` (PK = `(project, name)`) | sí |
| 03 context_sections | `INSERT OR IGNORE` (PK = `(project, section_name)`) | sí |
...
| V2 | `COUNT(*) FROM notes` == archivos `.md` (excluyendo `MEMORY.md`) |
| V3 | `COUNT(*) FROM context_sections` == 12 |
